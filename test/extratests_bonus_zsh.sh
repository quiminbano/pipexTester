...
EOF
	...
	} 2>&1 | /usr/bin/tee /testValgrind_bonus/test24.txt;
	{
		...
		echo "";
		echo "";
		/usr/bin/valgrind --leak-check=full --show-leak-kinds=all --undef-value-errors=no --error-exitcode=200 --track-fds=yes ./pipex here_doc END cat 'awk -F ";" "{print $1}"' outfile2 << EOF;
hello;friends
world;is
everyone;is
END
...
EOF
	...
	} 2>&1 | /usr/bin/tee /testValgrind_bonus/test25.txt;
	{
		...
		echo "";
		echo "";
		/usr/bin/valgrind --leak-check=full --show-leak-kinds=all --undef-value-errors=no --error-exitcode=200 --track-fds=yes ./pipex here_doc END cat 'awk -F ";" "{print $1}"' 'grep hello' outfile2 << EOF;
hello;friends
world;is
everyone;is
END
