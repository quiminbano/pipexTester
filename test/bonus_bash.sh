...
while [ $index -le $sizeArray ]; do
	...
	} &> test${index}_bonus.txt;
	mv test${index}_bonus.txt testValgrind_bonus/
	echo -e "${yellow}Done. Check the file test${index}.txt inside the folder testValgrind_bonus to see the results.${nocolor}"
	echo "";
	echo "";
	((index++));
	sleep 2;
...
mv test17_bonus.txt testValgrind_bonus/
{
	echo ""
	echo ""
