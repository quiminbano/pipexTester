...
while [ $index -lt $sizeArray ]; do
	...
	fi
	{
		...
		if [ $index -eq 4 ] || [ $index -eq 11 ]; then
		...
		returnPipex=$?
		after=$(date +%s);
		if [ $index -eq 6 ] || [ $index -eq 7 ] || [ $index -eq 11 ] || [ $index -eq 24 ] ; then
			echo -ne "${yellow}Checking the execution time of pipex in test $index: ${nocolor}";
			diffTime=$((after - before));
			if [ $index -eq 11 ] && ([ $diffTime -lt 1 ] || [ $diffTime -gt 6 ]); then
		...
		echo "";
		if [ $index -eq 5 ] && ([ $index -gt 7 ] && [ $index -ne 11 ] && [ $index -ne 24 ]); then
